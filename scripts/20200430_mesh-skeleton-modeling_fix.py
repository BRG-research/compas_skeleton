...
from compas_rhino.selectors import network_select_vertex
from compas_rhino.modifiers import network_move_vertex
...
def generate_section(radius, reference_plane, target_plane, reverse):
    ...
    return orient_points(points, reference_plane=reference_plane, target_plane=target_plane)
...
def generate_node(network, key, radius, fac):
    ...
    pt_cent = network.node_coordinates(key)
    ...
    for u, v in edges:
        ...
        vec_nbr = subtract_vectors(network.node_coordinates(u), network.node_coordinates(v))
        ...
        points = generate_section(radius, ref_plane, [pt_1, normalize_vector(vec_nbr)], flag)
        inner_key = [mesh.add_vertex(x=x, y=y, z=z) for x, y, z in points]
        # create outer cross section points
        points = generate_section(radius, ref_plane, [pt_2, normalize_vector(vec_nbr)], flag)
    ...
    faces_index = convex_hull(points)  # need mapping, index is from 0 to length of points
...
fac = 0.25  # 0.25      # global scale for smooth corners
...
while True:
    ...
    if key is not None:
        ...
        network_move_vertex(network, key)
    else:
        break
